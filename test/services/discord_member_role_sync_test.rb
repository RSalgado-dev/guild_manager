...
class DiscordMemberRoleSyncTest < ActiveSupport::TestCase
  ...
  test "preserva cargo máximo local sem role do Discord" do
    user = users(:five)
    guild = user.guild
    maximum_role = Role.create!(
      guild: guild,
      name: "Cargo Máximo Local",
      description: "Cargo app-only para acesso máximo",
      category: "role_maximum",
      managed_by_app: false,
      discord_role_id: nil
    )
    user.user_roles.create!(role: maximum_role)
    ...
    assert DiscordMemberRoleSync.call(user:, guild:, client: DiscordApiClient.new(bot_token: "fake_bot_token"))
    assert user.reload.roles.exists?(id: maximum_role.id)
  end
